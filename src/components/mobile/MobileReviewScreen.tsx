  ...
  return (
    <div className="flex flex-col h-screen bg-background">
      <div className="bg-card border-b border-border px-4 pt-12 pb-3 flex items-center gap-3">
        ...
        <div>
          <h1 className="text-base font-bold text-foreground">Review Order</h1>
          <p className="text-xs text-muted-foreground">Table {table.number} · <span className="capitalize">{serviceMode}</span></p>
        </div>
      </div>
            ...
            <div key={item.id} className="bg-card rounded-xl border border-border p-4">
              <div className="flex justify-between items-start mb-2">
                <div>
                  ...
                  {item.notes && <p className="text-xs text-pos-occupied mt-0.5">📝 {item.notes}</p>}
                </div>
              ...
              <div className="flex items-center gap-3">
                <button onClick={() => onUpdateQty(item.id, -1)}
                  className="w-8 h-8 rounded-lg bg-muted flex items-center justify-center hover:bg-secondary">
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-card border-t border-border p-4 space-y-2">
        ...
        <div className="flex justify-between text-sm text-muted-foreground">
          <span>Service (10%)</span><span>${totals.serviceCharge.toFixed(2)}</span>
        </div>
        <div className="flex justify-between text-sm text-muted-foreground">
          <span>GST (9%)</span><span>${totals.gst.toFixed(2)}</span>
        </div>
        <div className="flex justify-between text-base font-bold text-foreground pt-2 border-t border-border">
          <span>Total</span><span>${totals.total.toFixed(2)}</span>
        </div>
        <Button variant="pay" size="xl" className="w-full mt-2" onClick={onPay}>
