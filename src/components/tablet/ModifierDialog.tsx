...
export const ModifierDialog: React.FC<ModifierDialogProps> = ({ item, groups, onConfirm, onCancel }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-foreground/40 animate-fade-in" onClick={onCancel}>
      <div className="bg-card rounded-xl shadow-xl w-full max-w-md mx-4 animate-slide-up" onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between p-4 border-b border-border">
          <div>
            <h3 className="font-semibold text-foreground">{item.name}</h3>
            <p className="text-sm text-muted-foreground">${item.price.toFixed(2)}</p>
          </div>
          <button onClick={onCancel} className="p-1.5 rounded-md hover:bg-muted text-muted-foreground">
            <X className="h-4 w-4" />
          </button>
        </div>
        ...
        <div className="p-4 max-h-[60vh] overflow-y-auto pos-scrollbar space-y-5">
          {groups.map(group => (
            <div key={group.id}>
              <div className="flex items-center gap-2 mb-2">
                ...
                {group.required && (
                  <span className="text-[10px] font-medium text-destructive bg-destructive/10 px-1.5 py-0.5 rounded">Required</span>
                )}
                  ...
                  return (
                    ...
                    >
                      ...
                      <span className="flex items-center gap-1">
                        {opt.price > 0 && <span className="text-xs text-muted-foreground">+${opt.price.toFixed(2)}</span>}
          ...
          {/* Notes */}
          <div>
            <span className="text-sm font-medium text-foreground mb-2 block">Special Notes</span>
            <textarea
              value={notes}
              onChange={e => setNotes(e.target.value)}
              placeholder="e.g. No onion, extra chilli..."
              className="w-full h-16 px-3 py-2 rounded-md bg-muted text-sm text-foreground placeholder:text-muted-foreground border-0 focus:outline-none focus:ring-1 focus:ring-ring resize-none"
            />
          </div>
        </div>
        ...
        <div className="p-4 border-t border-border flex gap-2">
          ...
          <Button onClick={handleConfirm} disabled={!isValid} className="flex-1">Add to Order</Button>
        </div>
      </div>
    </div>
  );
