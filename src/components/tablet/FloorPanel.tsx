...
import { Search, Plus, Utensils, ShoppingBag, Truck } from "lucide-react";
...
const statusColors: Record<string, string> = {
  available: "bg-pos-available/15 border-pos-available/40 text-pos-available",
  occupied: "bg-pos-occupied/15 border-pos-occupied/40 text-pos-occupied",
  reserved: "bg-pos-reserved/15 border-pos-reserved/40 text-pos-reserved",
  dirty: "bg-pos-dirty/15 border-pos-dirty/40 text-pos-dirty",
};
  ...
  return (
    <div className="w-64 bg-card border-r border-border flex flex-col shrink-0">
      ...
      <div className="p-4 border-b border-border">
        <h2 className="font-semibold text-foreground text-sm">Floor</h2>
          ...
          {filteredTables.map(table => (
            ...
            >
              <div className="flex items-center justify-between mb-1">
                <span className="font-semibold text-foreground text-sm">T{table.number}</span>
                <span className={cn("w-2 h-2 rounded-full", statusDot[table.status])} />
              </div>
      ...
      <div className="p-3 border-t border-border space-y-1.5">
        <Button
          ...
          onClick={() => onCreateWalkIn("takeaway")}
        >
        ...
        <Button
          ...
          size="sm"
          className="w-full justify-start gap-2 text-xs"
          onClick={() => onCreateWalkIn("delivery")}
        >
